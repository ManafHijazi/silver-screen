   ...
   <div className={`box-theme-component-wrapper${(isDragOver && ' drag-over') || ''}`}>
     <div className="dropzone-wrapper">
       <LoadableImageComponant
         src={
          (file && getMimeTypeHandler(file.fileName).image)
          || (defaultImage
       ...
       />
       {file && file.status === 'uploading' && <span className="mdi mdi-reload mdi-spin as-overlay-spinner" />}
       ...
       <Button className="btns theme-solid mx-0" onClick={() => uploadRef.current.click()}>
         <span className="mx-3 text-nowrap">
           {(accept
            ...
            && t(
              `${translationPathShared}${(multiple && 'browse-images') || 'browse-image'}`
            ))
            || t(
              `${translationPathShared}${(multiple && 'browse-files') || 'browse-file'}`
            )}
         </span>
       </Button>
       {file && (
